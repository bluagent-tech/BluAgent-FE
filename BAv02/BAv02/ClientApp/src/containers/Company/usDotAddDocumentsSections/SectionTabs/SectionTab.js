...
class SectionTab extends React.Component {
  ...
  onProcessFile() {
    let { onProcessFile } = this.props;
    let pondFiles = this.pond.getFiles();
    if (onProcessFile) onProcessFile(pondFiles);
  }

  onfilesUpdate(files) {
    let disable = files.length > 0;
  ...
  uploadFile() {
    ...
    let timeout = 5000;
    if (files.length !== 0) {
      const elementId = 'loading' + this.props.tabId
