...
class UploadDriversFilesHazmatCertificate extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    this.props.getAllTrainingDocuments(id, 1, 100);
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevProps.docsTraining !== this.props.docsTraining) {
      //Files.data = this.props.docsDrivers.filter(this.props.filter);
      Files.data = this.props.docsTraining.filter(
        (data) =>
          data.DocType === "TrainingCertificate" &&
          data.IdDriver === parseInt(IdNumber)
      );
  ...
  uploadFile() {
    ...
    if (files.length !== 0) {
      ...
      this.props.uploadTrainingCertificate(form);
      setTimeout(() => {
        this.toggle();
      }, timeout);
    } else {
  ...
  deleteDoc = () => {
    this.props.DeleteDocTrainingCert(
      this.state.idDelete,
      DriverId,
