...
class FileUploadTrainingCertificate extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      open: false,
    ...
    this.traningUploadFile = this.traningUploadFile.bind(this);
    this.openDocUrl = this.openDocUrl.bind(this);
    this.toggle = this.toggle.bind(this);
    this.getTrainingCerts = this.getTrainingCerts.bind(this);
    this.handlePageClick = this.handlePageClick.bind(this);
  }

  getTrainingCerts(IdCompany) {
    axios
      .get(
  ...
  handlePageClick(e) {
    ...
    this.setState(
      ...
      () => {
        this.getTrainingCerts();
      }
    );
  }

  componentDidMount() {
    this.getTrainingCerts(IdCompany);
  }
  ...
  traningUploadFile() {
    ...
    if (files.length !== 0) {
      ...
      this.props.trainingCertificateUploadFile(form);
      setTimeout(() => {
        this.toggle();
      }, timeout);
    } else {
  ...
  render() {
    const { documentExpireDate, countFiles } = this.state;
    return (
      <>
        <input
          type="image"
                  ...
                  {this.props.isDateExpire === "TRUE" ? (
                    <Col md="4">
                      <DatePicker
                        ...
                        value={dateConvertTables(documentExpireDate)}
              ...
              <Row>
                <Col md={{ size: 6, offset: 6 }} style={{ textAlign: "right" }}>
                  <button
                    ...
                    type="button"
                    onClick={this.traningUploadFile}
