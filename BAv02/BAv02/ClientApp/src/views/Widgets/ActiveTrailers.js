...
const ActiveTrailers = () => {
    ...
    const [activeTrailers, setActiveTrailers] = useState(0); // ? UseState para el valor de los Trailers activos
    ...
    const active = useSelector((state) => state.maintenance.trailers); // ? estado de trailers
    const isLoading = useSelector((state) => state.maintenance.isLoadingActiveTrailers); // ? Estado de la carga de datos

    // * UseEffect para llamar a una funion del store de Maintenance
    useEffect(() => {
        dispatch(actionCreatorsMaintenance.getTrailers(idUser, 1, 1000, true));
    }, []);

    // * Calculamos la cantidad de Trailers activos y se lo asignamos a la variable ActiveTrailers
    useEffect(() => {
        var total = 0;
        active.forEach(() => {
            total++;
        });
        setActiveTrailers(total);
    }, [active]);
